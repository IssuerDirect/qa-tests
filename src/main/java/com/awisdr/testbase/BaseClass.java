...
public class BaseClass {
	...
	/*
	 * protected static WebDriver driver; public static Logger log =
	 ...
	 */
	 ...
	 protected static WebDriver driver; public static Logger log =
	  ...
	  /* @Author: SreeGuntaka
		 * Feature : Generic
		 * @Description: This method Initiate Browser Driver 
		   Ex: initializeDriver()*/
	  public WebDriver initializeDriver() throws IOException {
	    try {
	      clearLogs();
	   //  String browserName = readPropertiesFile("browser", "//src//main//resources//data.properties");
	      String browserName = "chrome";
	      String workingDirChrome = System.getProperty("user.dir")+"\\NewDrivers";
	      Path filePathChrome = Paths.get(workingDirChrome + File.separator + "//chromedriver.exe");
	  ...
	  /* @Author: SreeGuntaka
		 * Feature : Generic
		 * @Description: This method Launch browser with the specified URL 
		   Ex: initiateBorwserDriver()*/
	  public WebDriver initiateBorwserDriver() throws IOException,InterruptedException {
	    try {
	      Thread.sleep(5000);
	      closeAllBrowsers();
	      driver = initializeDriver();
	      log.info("Driver is initialized");
	      String browserName = readPropertiesFile("clientURL", "//src//main//resources//configs//configuration.properties");
	      driver.get(browserName);
	      log.info("Navigatted to Home page");
	    }
	    catch(Exception e) {
	      System.out.println("Exception occured due to : " + e.toString());
	    }
	  ...
	  /*public Properties loadProperites() throws Exception{
			...
			String filePath = System.getProperty("user.dir")+"allure.properties";
	  ...
	  /* @Author: SreeGuntaka
		 * Feature : Generic
		 * @Description: This method reads the data from the Property file which is attached in the project location 
		   Ex: readPropertiesFile(String strGetValue, String strLocation)*/
	  ...
	  /* @Author: SreeGuntaka
		 * Feature : Generic
		 * @Description: Close existing browsers 
		   Ex: closeAllBrowsers()*/
	  ...
	  public void clearDownLoadedFIles() {
	    try {
	      ...
	      for (int i = 0; i < dirContents.length; i++) {
	        if (dirContents[i].getName().startsWith("definition-") || dirContents[i].getName().contains("COVERSHEET") || dirContents[i].getName().contains("SCT")
	        		|| dirContents[i].getName().contains("RCL") || dirContents[i].getName().contains("CICL")|| dirContents[i].getName().contains("CRL")
	        		|| dirContents[i].getName().contains("STATISTICS")|| dirContents[i].getName().contains("VALIDATIONREPORT")|| dirContents[i].getName().contains("DEF")) {
	  ...
	  /* @Author: SreeGuntaka
		 * Feature : Generic
		 * @Description: Captures screenshot if test case failed
		   Ex: captureScreenshot(WebDriver driver, String strTestCaseName)*/
